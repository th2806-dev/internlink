  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.successCount > 0) {
        onShowToast(
          `Import thành công ${res.successCount}/${res.totalRows} sinh viên · Có thể cấp tài khoản sau trong danh sách`,
        );
        onSuccess();
      } else {
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                Định dạng hỗ trợ: .xlsx, .xls • Tạo hồ sơ sinh viên từ file Excel
              </p>
            </div>
          </div>
                    ...
                    {result.errors.map((err, idx) => (
                      <div key={idx} className="text-[11px] text-rose-800 bg-white p-2 rounded border border-rose-200/60 flex items-start gap-2">
                        ...
                        <span className="flex-1">
                          {err.studentCode ? <strong>{err.studentCode}: </strong> : null}
                          {err.message}
                        </span>
                      </div>
                    ))}
